...
public strictfp class RobotPlayer {
	...
	static final float MAP_LARGE = 150f;						//Max size of map considered 'large'
			...
			switch (rc.getType()) {
				case ARCHON:
					...
					if ((area_trees/ARCHON_SIGHT_AREA) > FOREST_DENSITY_THRESHOLD){
						rc.broadcast(DENSE_FOREST_DETECTED ,1);
						rc.broadcast(BUILD_ORDER_QUEUE+1,2);	//Lumberjack
					}
					else{
						rc.broadcast(BUILD_ORDER_QUEUE+2,3);	//Soldier
					}

					//Setup build order
					rc.broadcast(BUILD_ORDER_QUEUE,1);	//Scout
