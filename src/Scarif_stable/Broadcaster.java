...
public class Broadcaster {
    ...
    //BFS to find closest un-updated grid for recce
    ...
    /*
    The Broadcaster class is one of our bot's secret weapons. It provides shared map awareness/vision in an environment that
    doesn't explicitly allow for such data sharing.

    As this year's competition is on continuous space, we discretize the environment into grids of size 10x10. (purely for space-constraints)
        --> After the upgrade of broadcasting channels to 10k, could've done 2.5x2.5 for more accuracy (1.6k channels per graph)
        --> Oh wells, didn't want to break our build with such a huge change

    Using such a system, we have 32bits of memory (1 Integer) per grid square to use to share location-specific information that persists in a game
    With this system in place, our soldiers were able to do 2 main things:
        --> Effective non-overlaping scouting
            --> Able to disperse to different locations that were not previously scouted (using bfs on grid to quickly acquire such locations)
        --> Engaging enemy on multiple fronts
            --> Given stored enemy deployment message, we can evaluate which location to respond to
            --> Thus only a subset of soldiers would respond to any given message (unless there's only 1)

    Further improvements to the current system that we thought about but didn't have enough time to implement:
        --> Reporting who's going where
            --> Would dramatically increase army efficiency as explicit allocation of forces would be available
        --> Using the map awareness as a metric to adapt our spawning conditions to
            --> Say map has tons of trees! And we're in an isolated position (think Abstract Art)
                --> Focus on economy, don't spawn soldiers
            --> Large maps, perhaps we can focus on building up a massed force of tanks
                --> A massive force of tanks beats a ball of soldiers through our testing
                --> Main problem was building up that critical mass of tanks tho, cuz on some maps, agile soldiers were
                    able to take out the lone tank.
    */
    ...
    //Initializes variables already computed and stored to global array
    ...
    //First-time initialization (run by first robot in execution order - basically just the starting Archon)
    private void setInitialize() throws GameActionException {
        rc.broadcast(BROADCASTER_INITIALIZED, 1);
        //Space-saving hueristic - since map is at most 100x100 in size, we reserve 200x200 space around robot's location
        OFFSET_X = Math.round(rc.getLocation().x/GRID_SIZE)-MAX_SIZE/2;
        OFFSET_Y = Math.round(rc.getLocation().y/GRID_SIZE)-MAX_SIZE/2;
    ...
    //Run by each navigating robot to constantly update the map boundaries
    ...
    //Writes to array index for the given location
    ...
    //Reads message at location's array index
    ...
    //Checks validity of tested location
    ...
    //Return nearest valid grid point - sometimes, given location is out of bounds when approximated to nearest 10
    //In such cases, returns nearest valid grid location instead
    ...
    //Conversions between location <=> channel <=> grid (serialized for broadcasting)
