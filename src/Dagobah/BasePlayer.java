...
public class BasePlayer {
    ...
    //Movement finesse
    static final int MOVEMENT_GRANULARITY = 15;      			//Number of possible move directions
    ...
    static final int SENSOR_GRANULARITY = 48;                   //How many angles to scan around
    ...
    static final int FARMER_MIN_SLOTS = 5;          			//Minimum number of trees that can be built before starting farm [1-6]
    ...
    static final float TANK_MICRO_DIST = 64f;                   //Squared dist. Distance nearest enemy has to be to engage micro code
    ...
    protected void endRound() throws GameActionException {
        ...
        //Update grid pos
        if (SURROUNDING_ROBOTS_ENEMY.length == 0) sitRep();
    ...
    protected boolean isSpreadOut(MapLocation myLoc, float radius){
        ...
        //Avoid gardeners
        for (RobotInfo f:farmers) if (f.type == RobotType.GARDENER) return false;
        return true;
    }
...
//        rc.setIndicatorDot(MY_LOCATION.add(initDir, MOVEMENT_SCANNING_DISTANCE), 0, 255, 0);
                ...
                /*if (!stuck) {
                    for (int i = 1; i < BASHING_GRANULARITY; i++) {
                        ...
                        if (rc.canMove(tryDir)) {
                            rc.move(tryDir);
                            rc.setIndicatorDot(MY_LOCATION.add(tryDir, MOVEMENT_SCANNING_DISTANCE), 255, 255, 0);
                            return;
                        }
    ...
    private Direction closest_direction_to_target(int GRANULARITY, int sensorDir, float distance, boolean rightBot){
        if (rightBot) {
            for (int i = 1; i < GRANULARITY; i++) {
                Direction tryDir = new Direction(SENSOR_ANGLES[absIdx(sensorDir + i, GRANULARITY)]);
...
//            }
    ...
    protected Direction getLeastDenseDirection(ArrayList<Direction> test_dir) throws GameActionException {
        ...
        for(int i=0;i<test_dir.size();i++){
            float eval = getDensity(MY_LOCATION.add(test_dir.get(i),MOVEMENT_SCANNING_DISTANCE),SENSE_SURROUNDING_DENSITY_RADIUS, TREES);
            if (!rc.onTheMap(MY_LOCATION.add(test_dir.get(i),MOVEMENT_SCANNING_DISTANCE), TEST_ON_MAP_RADIUS)) continue;
            if(eval<density){
                density = eval;
