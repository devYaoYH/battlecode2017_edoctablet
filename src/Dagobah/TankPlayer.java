...
public class TankPlayer extends BasePlayer {
    ...
    protected void moveToTargetExperimental(MapLocation myLoc, int GRANULARITY, boolean rightBot, boolean stuck){
        ...
        TreeInfo[] neutralTrees = rc.senseNearbyTrees(RobotType.TANK.bodyRadius+RobotType.TANK.strideRadius+EPSILON, Team.NEUTRAL);
...
//        rc.setIndicatorDot(MY_LOCATION.add(initDir, MOVEMENT_SCANNING_DISTANCE), 0, 255, 0);
        ...
        try {
            ...
            if (canMoveTank(initDir, nearbyTrees, neutralTrees)){
                rc.move(initDir);
                return;
            }
            else {
                ...
                if (rightBot) {
                    for (int i = 1; i < GRANULARITY; i++) {
                        ...
                        if (canMoveTank(tryDir, nearbyTrees, neutralTrees)) {
                            rc.move(tryDir);
    ...
    private boolean canMoveTank(Direction tryDir, TreeInfo[] nearbyTrees, TreeInfo[] nearbyNeutrals){
