...
public class ArchonPlayer extends BasePlayer {
    ...
    private void initPlayer() throws GameActionException {
        ...
        //todo announces map size to other robots
    ...
    public void startRound() throws GameActionException {
        ...
        //Get cumulative smoothened number of units
        if (smoothened_settled_count.size() >= numberSmoothening){
            //todo implement a proper queue now...
...
//        int surrGardenerCounter = 0;
//        RobotInfo[] surr_gardeners = rc.senseNearbyRobots(5f, rc.getTeam());
//        for (RobotInfo r:surr_gardeners){
//            if (r.type == RobotType.GARDENER) surrGardenerCounter++;
//        }
//        if (surrGardenerCounter >= 3) return false;     //Don't overcrowd
    	if (rc.getRoundNum() < 400 && rc.getRoundNum() > 100 && number_soldiers == 0 && number_gardeners != 0){
    		return false;
    	}
        if (est_size < MAP_SMALL && rc.getTeamBullets() < 300){ //Go econ if u have a decent economy
            //On small maps, prioritize offense
            if (enemyEngaged && number_gardeners >= 1 && number_soldiers < 3){
                return false;
            }
        }
        //todo timer cooldown for spawning gardeners
        if (rc.getRoundNum() < gardenerTimer){
            if (rc.getTeamBullets() < 347f) return false;
        }
        if (number_gardeners < MIN_GARDENERS) return true;
            ...
            if(number_gardeners > 0.05f+(5f/(float)Math.max(1,number_gardeners_settled))) return false;
        }
        return true;
    }
    ...
    private void jiggleArchon() throws GameActionException {
        ...
        for (int j=1;j<SLUG;j++) {
            tryDist = rc.getType().strideRadius/(float)Math.pow(3,SLUG);
