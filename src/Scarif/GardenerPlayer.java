...
public class GardenerPlayer extends BasePlayer {
    ...
    static final int TANK_DELAY = 17; 							//Number of turns to spawn a new tank
    ...
    private void run() throws GameActionException {
        ...
        else if (current_available_slots.size() < 1){
            if (SPAWN_DIR != null){
    ...
    private boolean isSettleLocationValid(MapLocation loc) throws GameActionException {
        ...
        if (Clock.getBytecodeNum() > BYTE_THRESHOLD) return true;
        ...
        if (rc.canSenseAllOfCircle(loc, rc.getType().bodyRadius)) {
            ...
            else if (rc.isLocationOccupiedByRobot(loc)) {
            ...
            else if (rc.isCircleOccupiedExceptByThisRobot(loc, rc.getType().bodyRadius)) {
    ...
    private float gardenerSurroundingDensity(MapLocation travelLoc) throws GameActionException {
        ...
        for (int i=0;i<6;i++){
            ...
            else curDensity += getDensity(testLoc, 1f, TREES);
        }
        return curDensity;
    }
