...
public class RushPlayer {
    ...
    static final int MIN_ACCEPTABLE_TREES = 2; //Min number of trees that gardener can plant
    ...
    static final int EARLY_GARDENER_DELAY = 30; //Number of turns to spawn a new gardener
    ...
    static final int END_GAME = 1500;
    ...
    static int UNIT_DELAY = 100; //Normal delay for spawning units
     ...
     * LilForts
     * OMGTree
     * [DigMeOut => Not detected as small map]
     * Shrine
     */
    ...
    **/
    ...
    static void runArchon(){
		...
		//Spawn Code
    	if ((num_gardener < 1 || rc.getTeamBullets() >= 250f) && rc.getRoundNum() >= gardenerTimer){
    		try {
	...
	static void runGardener(){
		...
		if (!settledDown){
			...
			/* 1 lumberjack 1 soldier*/
			if (rc.getRoundNum() <= 20 && num_lumberjack < 1){ //Spawn soldier for early defence
				spawnUnitRandom(20, RobotType.LUMBERJACK);
			}
			if (rc.getRoundNum() <= 20 && num_soldier < 2){ //Spawn soldier for early defence
	...
	static void runLumberjack(){
		...
		if (inCombat){
			...
			if (nearestdist <= LUMBERJACK_MICRO_DIST || nearbyBullets.length > 0){
	...
	//moveToTarget for lumberjacks
	static void clearToTarget(MapLocation myLoc){ 
    	...
    	Direction initDir = rc.getLocation().directionTo(target); //Direction chosen 
    	MapLocation tgtLoc = myLoc.add(initDir, RobotType.LUMBERJACK.strideRadius); //Target location to move to
    	rc.setIndicatorDot(tgtLoc, 255, 0, 0);
		if (rc.canMove(initDir)){ //Move directly to location
			try {
				rc.move(initDir);
			} catch (GameActionException e) {
				e.printStackTrace();
		...
		/*if (rc.canChop(tgtLoc)){ //Chop to location
			try {
				rc.chop(tgtLoc);
			} catch (GameActionException e) {
				e.printStackTrace();
		...
		//Chop to location if failure to canChop
		TreeInfo [] nearbyTrees = rc.senseNearbyTrees(GameConstants.INTERACTION_DIST_FROM_EDGE + RobotType.LUMBERJACK.bodyRadius, rc.getTeam().NEUTRAL);
		//System.out.println("Sensing " + nearbyTrees.length + " trees");
		float nearestDist = 1000000f;
		TreeInfo bestTree = null;
		for (TreeInfo tree : nearbyTrees){
			float dist = tree.getLocation().distanceTo(tgtLoc) - tree.getRadius();
			if (dist < nearestDist){
				nearestDist = dist;
				bestTree = tree;
			}
  ...
  //A heuristic to help lumberjacks move in combat
    static void lumberjackCombatMove(RobotInfo nearestEnemy, RobotInfo[] nearbyEnemies, BulletInfo[] nearbyBullets){
		...
		if (rc.canChop(tgtLoc)){ //Chop to location
			...
			//Chop to location if failure to canChop
			TreeInfo [] nearbyTrees = rc.senseNearbyTrees(GameConstants.INTERACTION_DIST_FROM_EDGE + RobotType.LUMBERJACK.bodyRadius);
			float nearestDist = 1000000f;
			TreeInfo bestTree = null;
			for (TreeInfo tree : nearbyTrees){
				float dist = tree.getLocation().distanceTo(tgtLoc);
				if (dist < nearestDist){
					nearestDist = dist;
					bestTree = tree;
				}
  ...
  //This assumes that rc.canMove returned false. Returns false when not a tank
    ...
    static void donate(){
    	...
    	if (getGamePhase() == GamePhase.END_GAME || getGamePhase() == GamePhase.FINAL_PUSH){
    		try {
  ...
  //Archon score for micro.
    ...
    static RobotType getNextUnitType(){
    	...
    	try{
	    	...
	    	else if (getGamePhase() == GamePhase.EARLY_GAME){
	    		if (num_lumberjacks < 1){ //If lumberjack somehow gets killed
	    	...
	    	else if (getGamePhase() == GamePhase.MID_GAME || getGamePhase() == GamePhase.END_GAME || getGamePhase() == GamePhase.FINAL_PUSH){
	    		return RobotType.SOLDIER;
	    	}
    	}
    	catch (GameActionException e){
